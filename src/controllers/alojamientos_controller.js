...
const crearAlojamiento = async (req, res) => {
  try {
    ...
    if (!titulo || !descripcion || !precioNoche || !maxHuespedes) return res.status(400).json({ msg: "Faltan campos obligatorios" });
    if (precioNoche <= 0) return res.status(400).json({ msg: "El precio debe ser un número positivo" });
    if (maxHuespedes <= 0) return res.status(400).json({ msg: "El número mínimo de huéspedes debe ser mayor a 0" });
    if (titulo.length > 50) return res.status(400).json({ msg: "El título no puede tener más de 50 caracteres" });
    if (!Number.isInteger(maxHuespedes)) return res.status(400).json({ msg: "El número de huéspedes debe ser un número entero" });
...
const obtenerAlojamientoPorId = async (req, res) => {
  try {
    ...
    if (alojamiento.estadoAlojamiento !== 'activo') return res.status(403).json({ msg: "Este alojamiento no está disponible actualmente" });
...
const actualizarAlojamiento = async (req, res) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(id))return res.status(404).json({ msg: `Lo sentimos, no existe el alojamiento con id ${id}` });
  ...
  try {
    ...
    if (String(alojamiento.anfitrion) !== String(req.usuario._id)) return res.status(403).json({ msg: "No tienes permiso para actualizar este alojamiento" });
...
const eliminarAlojamiento = async (req, res) => {
  ...
  if (!mongoose.Types.ObjectId.isValid(id)) return res.status(404).json({ msg: `Lo sentimos, no existe el alojamiento con ID inválido` });


  try {
    const alojamiento = await Alojamiento.findById(id);
    if (!alojamiento) return res.status(404).json({ msg: "Lo sentimos, el alojamiento no existe" });

    // Verificar que el usuario autenticado sea el dueño del alojamiento
    if (String(alojamiento.anfitrion) !== String(req.usuario._id)) return res.status(403).json({ msg: "No tienes permiso para eliminar este alojamiento" });
